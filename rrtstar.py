...
class RRT_Solver:
    ...
    start: Node        # node
    goal: BaseGeometry  # shapely polygon
    ...
    def __init__(
        ...
        self.tree.append(
            Node(startstate[0], startstate[1])
        )  # Initialize tree with start state
        self.start = self.tree[0]
        self.goal = goalregion
    ...
    def check_goal_reachable(self, pygame, screen) -> bool:
        ...
        goal_center = self.goal.centroid
        goal_node = Node(goal_center.x, goal_center.y)

        nn = self.tree[0]
        for p in self.tree:
            if distance(p, goal_node) < distance(nn, goal_node):
                nn = p

        if self.collision_check(nn, goal_node):
            goal_node.cost = nn.cost + distance(nn, goal_node)
            goal_node.set_parent(nn)
            self.tree.append(goal_node)
            print("[INFO]\t... reached goal sucessfully!\r\n")

            self.construct_path(goal_node, pygame, screen)
            return True
        else:
    ...
    def resolve(self, iter, pygame, screen) -> bool:
        for i in range(iter):
    ...
    def solve(self, pygame, screen):
        ...
        if not self.check_goal_reachable(pygame, screen):
            ...
            for j in range(3):
                if self.resolve(200, pygame, screen):
                    return True
            return False  
        return True